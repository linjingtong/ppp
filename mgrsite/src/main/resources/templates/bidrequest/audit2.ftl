...
<script type="text/javascript">
	$(function() {
		...
		$(".auditClass").click(function(){
            $("#audit2_dialog").dialog("open");
			var data=$(this).data("json");
			$("[name=id]").val(data.id);
			$("[name=username]").html(data.username);
			$("[name=title]").html(data.title);
			$("[name=bidRequestAmount]").html(data.bidRequestAmount);
			$("[name=currentRate]").html(data.currentRate);
			$("[name=monthes2Return]").html(data.monthes2Return);
			$("[name=totalRewardAmount]").html(data.totalRewardAmount);
		});
