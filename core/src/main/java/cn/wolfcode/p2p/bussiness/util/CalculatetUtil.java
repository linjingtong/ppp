package cn.wolfcode.p2p.bussiness.util;
		...
		if (returnType == BidConst.RETURN_TYPE_MONTH_INTEREST_PRINCIPAL) {// 按月分期
			...
			if (monthes2Return == 1) {
				...
				if (monthIndex < monthes2Return) {
					monthlyInterest = DecimalFormatUtil.amountFormat((temp1.subtract(monthToReturnMoney)).multiply(temp4).add(monthToReturnMoney).setScale(BidConst.CAL_SCALE, RoundingMode.HALF_UP));
